...
<section class="content">
		...
		<fieldset>
			<p>
				<label for="driver">
					...
					<span class="info">Database's driver.</span>
				</label>
				<select id="driver" class="chosen-select display-db" name="driver">
					<?php $drivers = ['mysql', 'sqlite']; ?>
					<?php foreach($drivers as $driver): ?>
					<?php $selected = strcmp($driver, 'mysql') == 0 ? ' selected' : ''; ?>
					<option<?php echo $selected; ?>><?php echo $driver; ?></option>
					<?php endforeach; ?>
				</select>
			</p>
		</fieldset>
	</form>
	<form id="db-sqlite" style="display:none" method="post" action="<?php echo uri_to('database_sqlite'); ?>" autocomplete="off">
		<?php echo $messages; ?>
		<input id="driver" name="driver" hidden value="sqlite">

		<fieldset>
			<p>
				<label for="database">Database Path</label>
				<?php
				$sqlitepath = explode("/", getcwd());
				array_pop($sqlitepath);
				$sqlitepath = implode("/", $sqlitepath)."/";
				?>
				<input id="database" name="path" value="<?php echo Input::previous('path', $sqlitepath.'blog.sqlite'); ?>">

				<i>The path to your database file.</i>
			</p>
	...
	<form id="db-mysql" method="post" action="<?php echo uri_to('database'); ?>" autocomplete="off">
		<?php echo $messages; ?>
		<input id="driver" name="driver" hidden value="mysql">
